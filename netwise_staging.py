...
contact_prod = file.file_to_str(os.getcwd() + "\\sql_files\\staging_to_production\\", "contact_prod.sql")
...
for col in cols_1:
    ...
    try:
        res = cursor.execute(query1.format('ig_staging.company', 'ig_dev.company_netwise_q3_b2', col))
...
for col in cols_2:
    print("Processing {}.".format(col))
    try:
        res = cursor.execute(query2.format('ig_staging.company', 'ig_dev.company_netwise_q3_b2', col))
        print(cursor.rowcount)
        connection.commit()
    except Exception as e:
        print(e)
...
for col in cols_3:
    print("Processing {}.".format(col))
    try:
        res = cursor.execute(query3.format('ig_staging.company', 'ig_dev.company_netwise_q3_b2', col))
        print(cursor.rowcount)
        connection.commit()
    except Exception as e:
        print(e)
